...
-- kasir access
GRANT SELECT, INSERT, UPDATE, DELETE ON TRANSAKSI, TRANSAKSI_DETAIL TO kasir;
GRANT SELECT, INSERT, UPDATE, DELETE ON MEMBER TO kasir;
GRANT SELECT ON BARANG, RAK_GUDANG, RAK_GUDANG_DETAIL, KODE_TRANSAKSI, DISKON TO kasir;

-- pemasok access
GRANT SELECT, INSERT, UPDATE, DELETE ON BARANG TO pemasok;

-- manager access
GRANT SELECT, INSERT, UPDATE, DELETE ON KASIR TO manager;
GRANT SELECT, INSERT, UPDATE, DELETE ON PEMASOK TO manager;
...
--------QUERY------------
--Query 1--
--Menampilkan data nomor dan nama pelanggan yang melakukan pembayaran dengan cashless, 
--beserta jumlah transaksi yang pernah dilakukannya. 
--Kemudian urutkan tampilannya berdasarkan jumlah transaksi paling banyak dan hanya tampilkan 10 pelanggan teratas

SELECT M.M_NO AS NO, M.M_NAMA AS NAMA, COUNT(T.T_NO) AS "JUMLAH TRANSAKSI" 
FROM MEMBER M 
JOIN TRANSAKSI T ON (M.M_NO = T.MEMBER_M_NO) 
WHERE M.M_NO != 'P000000' 
GROUP BY M.M_NO, M.M_NAMA 
ORDER BY "JUMLAH TRANSAKSI" DESC 
FETCH NEXT 10 ROWS ONLY;

--Query 2--
--Menampilkan nama pemasok, nama barang, dan harganya. 
--Urutkan berdasarkan nama pemasok dari terkecil dan harga dari harga yang terbesar.

SELECT Pem_perusahaan AS "NAMA PEMASOK", b_nama AS "NAMA BARANG", b_harga AS "HARGA BARANG" 
FROM Pemasok 
JOIN RAK_GUDANG ON (PEMASOK.Rak_Gudang_g_koderak = RAK_GUDANG.g_koderak) 
JOIN RAK_GUDANG_DETAIL ON (RAK_GUDANG.g_koderak = RAK_GUDANG_DETAIL.Rak_gudang_g_koderak) 
JOIN BARANG ON (barang_b_id = b_id) 
ORDER BY Pem_perusahaan ASC, b_harga DESC;

-------View--------

--View 1--
--View untuk menampilkan id transaksi, t.tgl pembelian, nama pelanggan dan nama kasir, 
--untuk semua transaksi yang dilakukan pada bulan Desember

CREATE OR REPLACE VIEW view_transaksi_desember AS
SELECT t.T_NO AS ID_TRANSAKSI, t.T_TGLPEMBELIAN AS TANGGAL_PEMBELIAN, m.M_NAMA AS NAMA_PELANGGAN, k.K_NAMA AS NAMA_KASIR
FROM TRANSAKSI t
JOIN MEMBER m ON t.MEMBER_M_NO = m.M_NO
JOIN KASIR_DETAIL kd ON t.T_NO = kd.TRANSAKSI_T_NO
JOIN KASIR k ON kd.KASIR_K_NO = k.K_NO
WHERE EXTRACT(MONTH FROM t.T_TGLPEMBELIAN) = 12;

SELECT * FROM VIEW_TRANSAKSI_DESEMBER;

--View 2--
--View yang menampilkan informasi transaksi bulanan yang dilakukan oleh member. 
--Dengan menampilkan jumlah transaksi, total pembelian, dan rata-rata pembelian per bulan.

CREATE OR REPLACE VIEW view_transaksi_bulanan AS
SELECT DATE_TRUNC('month', t.T_TGLPEMBELIAN) AS Bulan_Transaksi, 
       COUNT(DISTINCT t.T_NO) AS Jumlah_Transaksi, 
       SUM((b.B_HARGA - COALESCE(d.D_DISKON, 0)) * td.TD_BANYAK_BARANG) AS Total_Pembelian, 
       AVG((b.B_HARGA - COALESCE(d.D_DISKON, 0)) * td.TD_BANYAK_BARANG) AS Rata_Rata_Pembelian
FROM TRANSAKSI t
JOIN TRANSAKSI_DETAIL td ON t.T_NO = td.TRANSAKSI_T_NO
JOIN BARANG b ON td.BARANG_B_ID = b.B_ID
LEFT JOIN DISKON d ON b.B_ID = d.BARANG_B_ID
WHERE t.MEMBER_M_NO <> 'P000000'
GROUP BY DATE_TRUNC('month', t.T_TGLPEMBELIAN);

SELECT * FROM view_transaksi_bulanan;


-----FUNCTION------

--Function 1--
--Fungsi untuk menghitung jumlah transaksi yang dilakukan oleh seorang pelanggan--

CREATE OR REPLACE FUNCTION hitung_jumlah_transak(NAMAM VARCHAR)
RETURNS TABLE (nama VARCHAR, jumlah_transaksi INT) AS $$
BEGIN
    RETURN QUERY
    SELECT DISTINCT M.M_NAMA AS NAMA, COUNT(T.T_NO) ::INTEGER AS JUMLAH_TRANSAKSI
    FROM MEMBER M
    JOIN TRANSAKSI T ON M.M_NO = T.MEMBER_M_NO
    WHERE M.M_NAMA LIKE '%' || NAMAM || '%'
    GROUP BY M.M_NAMA;
END;
$$ LANGUAGE plpgsql;
...
---Cek Function 1---
INSERT INTO TRANSAKSI(T_NO, T_TGLPEMBELIAN, T_ISCASHLESS, MEMBER_M_NO )  
VALUES (64, TO_DATE ('2022-11-28 06:14:00','yyyy-mm-dd HH24:MI:SS'), '1', 'P000017');

INSERT INTO MEMBER (M_NO, M_NAMA, M_JENISKEL, M_TEMPATLAHIR, M_TANGGALLAHIR, M_TANGGALDAFTAR, M_ALAMAT, M_EMAIL, M_PASSWORD, M_NOTELP) 
VALUES 
    ('P000017','ANISA PUTRI', 'P','LAMONGAN', TO_DATE ('1975-09-10', 'yyyy-mm-dd'), TO_DATE('2022-05-09', 'YYYY-MM-DD'), 'MULYOREJO UTARA GG3 NO 12C', 'anisa1975@gmail.com', 'Nisacantik123', '081357627439');

SELECT * FROM hitung_jumlah_transak('ANISA');

--Function 2--
--Fungsi untuk menghitung total pembelian dari sebuah transaksi--

CREATE OR REPLACE FUNCTION HITUNG_TOTAL_PEMBELIAN(T_NO INT) RETURNS DECIMAL(10, 2) AS $$
DECLARE
  total DECIMAL(10, 2);
  member_m_no CHAR(10);
BEGIN
  SELECT t.MEMBER_M_NO INTO member_m_no FROM TRANSAKSI t WHERE t.T_NO = HITUNG_TOTAL_PEMBELIAN.T_NO;

  IF member_m_no = 'P000000' THEN
    SELECT SUM(b.B_HARGA * td.TD_BANYAK_BARANG)
    INTO total
    FROM TRANSAKSI t
    JOIN TRANSAKSI_DETAIL td ON t.T_NO = td.TRANSAKSI_T_NO
    JOIN BARANG b ON td.BARANG_B_ID = b.B_ID
    WHERE t.T_NO = HITUNG_TOTAL_PEMBELIAN.T_NO;
  ELSE
    SELECT SUM((b.B_HARGA - COALESCE(d.D_DISKON, 0)) * td.TD_BANYAK_BARANG)
    INTO total
    FROM TRANSAKSI t
    JOIN TRANSAKSI_DETAIL td ON t.T_NO = td.TRANSAKSI_T_NO
    JOIN BARANG b ON td.BARANG_B_ID = b.B_ID
...
---Cek Function 2---
SELECT hitung_total_pembelian(25);
...
---Trigger 1---
---Trigger untuk mengecek tanggal lahir kasir valid/tidak---
CREATE OR REPLACE FUNCTION trg_kasir_check_tgllahir()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.K_TGLLAHIR IS NULL OR NEW.K_TGLLAHIR > CURRENT_DATE THEN
        RAISE EXCEPTION 'Tanggal lahir kasir tidak valid';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_kasir_check_tgllahir
BEFORE INSERT OR UPDATE ON KASIR
FOR EACH ROW
EXECUTE FUNCTION trg_kasir_check_tgllahir();

--Cek Trigger 1--
UPDATE KASIR
SET K_TGLLAHIR = TO_DATE('2025-02-15', 'yyyy-mm-dd')
WHERE K_NO = '01';

---Trigger 2---
----Trigger untuk mengurangi stok barang pada tabel barang ketika ada data transaksi_detail baru ditambahkan.

CREATE OR REPLACE FUNCTION update_product_stock()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE BARANG
    SET B_STOK = B_STOK - NEW.TD_BANYAK_BARANG
    WHERE B_ID = NEW.BARANG_B_ID;
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER after_insert_transaksi_detail
...
INSERT INTO TRANSAKSI_DETAIL (TD_NO, TRANSAKSI_T_NO, TD_BANYAK_BARANG, BARANG_B_ID)  
VALUES (107, 60, 1, 'VX950');

INSERT INTO TRANSAKSI_DETAIL (TD_NO, TRANSAKSI_T_NO, TD_BANYAK_BARANG, BARANG_B_ID)  
VALUES (108, 61, 1, 'BB700');

INSERT INTO TRANSAKSI_DETAIL (TD_NO, TRANSAKSI_T_NO, TD_BANYAK_BARANG, BARANG_B_ID)  
VALUES (109, 60, 1, 'DF800');
